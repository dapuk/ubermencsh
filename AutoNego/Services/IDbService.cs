...
public interface IDbService
{
    ...
    Task<MySqlConnection> OpenAsync();
}

public sealed class DbService(IConfiguration cfg) : IDbService
{
    private readonly string _conn = cfg.GetConnectionString("MySqlServer")
        ?? Environment.GetEnvironmentVariable("ConnectionStrings__MySqlServer")
        ?? throw new InvalidOperationException("Missing connection string.");

    public async Task<MySqlConnection> OpenAsync()
    {
        var con = new MySqlConnection(_conn);
        await con.OpenAsync();
        return con;
    }
